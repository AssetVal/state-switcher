...
export const stateNameAndAbbrTuple = [
...
export default function abbreviateState(input, to) {
    ...
    const findAbbreviation = (stateStr) => {
        for (let i = 0; i < stateNameAndAbbrTuple.length; i++) {
            if (stateNameAndAbbrTuple[i][0] === stateStr)
                return stateNameAndAbbrTuple[i][1];
        }
        return undefined;
    };
    const findLongName = (stateStr) => {
        for (let i = 0; i < stateNameAndAbbrTuple.length; i++) {
            if (stateNameAndAbbrTuple[i][1] === stateStr)
                return stateNameAndAbbrTuple[i][0];
        }
        ;
        return undefined;
    };
    ...
    if (input !== undefined) {
        ...
        if (to === 'abbr' && input != undefined) {
            ...
            if (stateAbbreviations.includes(input))
                return input;
            return undefined;
        }
